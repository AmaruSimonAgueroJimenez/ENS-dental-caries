...
Earlier scripts (`ENScaries (2).R` and `RF con componentes principales.R`) leave `die11` as a factor after dropping unused levels; the later `index.qmd` converts it to numeric level indices. These are distinct historical representations. Neither step documents low/medium/high consumption cutoffs. The recovered dietary table presents water as a numeric mean and standard deviation, which does not establish the representation supplied to each fitted historical model. R explicitly warns that numeric conversion of factors returns their internal codes ([R factor documentation](https://search.r-project.org/R/refmans/base/html/factor.html)).
...
The original importance metric is a separate issue. The historical analysis agreements name Mean Decrease Gini, and the QMD does not explicitly enable permutation importance in random-forest fitting. The manuscript caption's reference to accuracy is therefore insufficient to verify the stored score's exact provenance. `randomForest` distinguishes permutation accuracy importance from impurity-based Gini importance ([official documentation](https://search.r-project.org/CRAN/refmans/randomForest/html/importance.html)). The revised unscaled, held-out Brier-score permutation values should not be compared numerically with the original rescaled bars. Differences in weighting, validation, sample selection and fitting also remain.
