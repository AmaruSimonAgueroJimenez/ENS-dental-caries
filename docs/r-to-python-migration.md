...
Earlier scripts (`ENScaries (2).R` and `RF con componentes principales.R`) leave `die11` as a factor after dropping unused levels; the later `index.qmd` converts it to numeric level indices. These are distinct historical representations. Neither step implements low/medium/high consumption cutoffs. Newly recovered written correspondence confirms the factor-to-numeric request on 24 September 2024 and proposes water bands on 22 November, after the October importance spreadsheet; no implementation or output for those bands was found. The recovered dietary table presents water as a numeric mean and standard deviation, which does not establish the representation supplied to each fitted historical model. R explicitly warns that numeric conversion of factors returns their internal codes ([R factor documentation](https://search.r-project.org/R/refmans/base/html/factor.html)).

### Controlled water-encoding sensitivity added on 10 September 2026

`water_encoding.py` compares reported glasses/day, the exact frozen historical index mapping and nominal one-hot categories for random forest and spline logistic regression. All six variants use the same 5,036 participants and five outer folds. The two physical-reference models reuse their saved probabilities, selected parameters and water permutations. Four alternative variants require 20 outer fits using the physical models' selected hyperparameters and seeds, without further tuning. Statistical preprocessing is learned in each outer training fold; the historical lookup is an explicitly full-source, data-derived mapping and is not claimed to have been learned in training.

Random forest has the same AUC (0.592248), probability scores and water importance for physical quantities and historical indices, within floating-point precision. The largest difference between individual probabilities is 3.33×10⁻¹⁶. Therefore this scale conversion does not explain the historical prominence of water in this fitted forest. Nominal RF gives ΔAUC −0.002593 (95% conditional interval −0.008556 to +0.003476); spline indices give −0.000408 (−0.000968 to +0.000126); nominal spline gives +0.001834 (−0.000824 to +0.004532). All four paired AUC intervals include zero, including the index RF contrast; this is not an equivalence test.

These comparisons isolate representation under fixed settings, not every consequence of the historical analysis. One-hot categories do not reproduce native R factor splitting and alter input dimensionality, potentially changing random-forest feature subsampling. Five common donor-row permutations per outer fold measure water importance, without implying new ranks against unpermuted predictors. Shared stratified PSU bootstrap intervals hold fitted models, hyperparameters and folds fixed. The source code and private caches are fingerprinted independently of the unchanged principal models.

The original importance metric is a separate issue. The newly recovered October and November discussions explicitly intend mean decrease in accuracy, superseding earlier Gini terminology. Nevertheless, the final QMD does not enable `importance=TRUE`; archived caret 6.0-94 defaults and the recovered 90-row, single-column `Overall` cache strongly support rescaled Gini under those preserved calls. The fitted RF object and its raw importance matrix were not recovered, so an exact runtime attribution cannot be certified. `randomForest` distinguishes permutation accuracy importance from impurity-based Gini importance ([official documentation](https://search.r-project.org/CRAN/refmans/randomForest/html/importance.html)).

All 11 original Word chart scores match the recovered spreadsheet exactly. Water is third (53.981911139722), as one numeric term among 90 encoded terms for 27 predictors. Seven numeric columns occupy the seven highest positions; the remaining 83 terms are category indicators. This unit of ranking differs from permuting each whole original predictor. The revised unscaled, held-out Brier-score permutation values should not be compared numerically with the original rescaled bars. Differences in weighting, validation, sample selection and fitting also remain. See the [historical analysis audit](historical-analysis-audit.md) for the chronology, source hashes, matched export and original legume-label correction.

### Original descriptive tables

The recovered 2024 source includes a separate 19-construct dietary table in addition to the sociodemographic table in the Word manuscript. The dietary header lists 4,998 participants, but its water statistics use 4,994 observed responses; four missing responses are not identified in that header. Revised tables use the full 5,036-person dentate cohort with observed and missing denominators by outcome column, survey-weighted column percentages or means and design-based intervals. All 27 predictor distributions are retained. Original unweighted significance tests are not reused for variable selection, and weekly beverage units and nonconsumer-specific portion denominators are made explicit before comparison.
